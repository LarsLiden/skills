...
- Compare computed styles before vs. after on representative elements. Practical techniques, in order of preference when available:
  ...
  - Diff a build of the compiled CSS. Use `git worktree add ../<tmp> <base-ref>` (or a separate clone) to build the pre-refactor output safely without disturbing working-tree changes, build the post-refactor output in the current tree, and diff the two. Do **not** use `git stash` for this — it risks losing uncommitted work and is fragile when untracked files or staged changes exist. After normalizing for whitespace/comments, the *declaration sets per selector* should match, or any difference must be explainable purely by selector consolidation that produces the same cascade outcome.
...
| Switching between equivalent-looking units (`16px` ↔ `1rem`) when extracting a variable | Do not convert units. Even if `1rem` currently equals `16px`, the root font size can be changed by users, by future code, or by media queries, and the computed pixel value will drift. Keep the exact original unit; if a codebase mixes both, extract two variables rather than unifying them |
